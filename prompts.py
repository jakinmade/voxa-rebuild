...
def apply_intent_mode(text: str, mode: str) -> str:
    ...
    mode_prompts = {
        "GET_IT_DONE": (
            "Tighten and restructure this text to genuinely match the writer's real sentence "
            "construction — see SENTENCE STRUCTURE, VOICE OBSERVATIONS, and ANCHOR SENTENCES above "
            "for what that actually looks like for this person. Restructuring FORM is expected, not "
            "just permitted: split a sentence, join two, reorder a clause, use a blunter pivot or a "
            "deliberate fragment if that's closer to their real style. None of that is 'inventing' — "
            "it's matching their construction. EXCEPTION, confirmed live: do not recast a statement "
            "as a question or a question as a statement (e.g. 'Curious if you got a chance to run "
            "it' becoming 'Did you get a chance to run it?') — this class of change necessarily drops "
            "the original framing word ('Curious') even when the underlying content survives, which "
            "counts as dropped content, not restructured form; keep the original sentence type. What "
            "is never permitted, regardless of restructuring: adding a new claim, fact, opinion, or "
            "idea not stated or directly implied in the input, or dropping/altering any factual "
            "detail, name, number, date, or commitment the input actually makes — including a framing "
            "word whose removal changes what's being conveyed. Content is fixed; form is not. "
            "Preserve the writer's voice: their directness, their cadence, their register. Do not add "
            "warmth, hedging, or polish that isn't already there."
        ),
        "WRITE_SOMETHING": (
...
def _build_system_prompt(
    ...
    base_rules = (
        ...
        "7. Every paragraph in the input gets a paragraph in the output — including the blank "
        "line between paragraphs. Do not merge two paragraphs into one, and do not compress into "
        "a summary.\n"
        + (
            "8. FORM is free, CONTENT is fixed. You may restructure sentences to match the "
            "writer's real construction — split one into two, join two into one, reorder clauses, "
            "use a blunter pivot or a deliberate fragment — wherever SENTENCE STRUCTURE/VOICE "
            "OBSERVATIONS/ANCHOR SENTENCES above show that's genuinely closer to how they write. "
            "EXCEPTION, confirmed live: never recast a statement as a question or a question as a "
            "statement (e.g. 'Curious if X' becoming 'Did you X?') — that necessarily drops the "
            "original framing word even when the underlying content survives, which is dropped "
            "content, not restructured form; keep the original sentence type. Otherwise, "
            "restructuring is not inventing, and is expected. What restructuring may NEVER do: add "
            "a new claim, fact, opinion, or idea not stated or directly implied by the input, or "
            "drop/alter any factual detail, name, number, date, commitment, or framing word whose "
            "removal changes what's being conveyed — every distinct point in the input must still "
            "be present in some form in the output, even if reshaped. There is no minimum word "
            "count — a shorter, more tightly-constructed result is the goal. Rule 7 (every "
            "paragraph AND its surrounding blank line preserved, no summarising, no merging "
            "paragraphs together) remains the safeguard at the paragraph level: reshape sentences "
            "freely, don't compress or merge whole paragraphs or ideas away.\n"
    ...
    if ai_score >= AI_CONTAMINATION_PATH_THRESHOLD:
        ...
        prompt = (
            ...
            "- LEXICAL FIDELITY — critical, but WORD-LEVEL only: this is not a paraphrase task for "
            "individual word choice. Preserve the writer's own words wherever they already fit the "
            "voice profile — do not substitute synonyms for variety ('if' vs 'whether', 'a while "
            "back' vs 'some time ago', etc) just for elegant variation. SENTENCE STRUCTURE is a "
            "separate axis and is explicitly NOT what lexical fidelity protects — see rule 8 below: "
            "restructuring (splitting, joining, reordering clauses, a blunter pivot, a deliberate "
            "fragment — but never recasting a statement as a question or vice versa, see rule 8's "
            "exception) toward the writer's real calibrated construction is expected wherever "
            "SENTENCE STRUCTURE/VOICE OBSERVATIONS/ANCHOR SENTENCES above show that's genuinely "
            "closer to their voice, even where the input's existing sentence boundaries were "
            "already grammatically fine. Lexical fidelity means: don't swap their actual words for "
            "synonyms, and don't add polish or hedging that isn't "
            "there. It does NOT mean: leave sentence boundaries, clause order, or sentence type "
            "untouched just because the input's phrasing already 'works.'\n"
...
def _grammar_fix_pass(text: str, client, locale: str = "uk", original_input_text: str = "") -> str:
    ...
    system = (
        ...
        "5. Missing articles (a, an, the) before countable nouns — including inside a "
        "comma-set-off appositive or parenthetical restating an earlier noun. Confirmed "
        "live: 'The scenario you described, dashboard green while an agent quietly does "
        "the wrong thing, is exactly what X is built to catch' — 'dashboard green' has no "
        "determiner and no verb, so it doesn't parse as a noun phrase; add 'a' ('a "
        "dashboard green while...') to fix it. This is a genuine missing-article error, "
        "not a deliberate compressed idiom like 'traffic light red' or 'all systems "
        "green' used as a standalone phrase — the distinguishing test is whether the "
        "phrase sits INSIDE a larger sentence as a restatement of a noun just mentioned "
        "(here, 'the scenario'), which needs the article to complete the grammar, versus "
        "standing alone as its own exclamation or heading, which doesn't. Do not treat "
        "every terse or compressed phrase as protected style by default — check whether "
        "adding the article actually changes meaning (it doesn't here) before leaving it.\n"
