...
def apply_intent_mode(text: str, mode: str) -> str:
    ...
    mode_prompts = {
        "GET_IT_DONE": (
            ...
            "just permitted: split a sentence, join two, reorder a clause, turn a statement into a "
            "question or vice versa, use a blunter pivot or a deliberate fragment if that's closer "
            "to their real style. None of that is 'inventing' — it's matching their construction. "
            "What is never permitted, regardless of restructuring: adding a new claim, fact, opinion, "
            "or idea not stated or directly implied in the input, or dropping/altering any factual "
            "detail, name, number, date, or commitment the input actually makes. Content is fixed; "
            "form is not. Preserve the writer's voice: their directness, their cadence, their "
            "register. Do not add warmth, hedging, or polish that isn't already there."
        ),
        "WRITE_SOMETHING": (
...
def _build_system_prompt(
    ...
    base_rules = (
        ...
        + (
            ...
            "turn a statement into a question or vice versa, use a blunter pivot or a deliberate "
            "fragment — wherever SENTENCE STRUCTURE/VOICE OBSERVATIONS/ANCHOR SENTENCES above show "
            "that's genuinely closer to how they write. That is restructuring, not inventing, and "
            "is expected. What restructuring may NEVER do: add a new claim, fact, opinion, or idea "
            "not stated or directly implied by the input, or drop/alter any factual detail, name, "
            "number, date, or commitment the input actually makes — every distinct point in the "
            "input must still be present in some form in the output, even if reshaped. There is no "
            "minimum word count — a shorter, more tightly-constructed result is the goal. Rule 7 "
            "(every paragraph preserved, no summarising) remains the safeguard at the paragraph "
            "level: reshape sentences freely, don't compress whole paragraphs or ideas away.\n"
    ...
    if ai_score >= AI_CONTAMINATION_PATH_THRESHOLD:
        ...
        prompt = (
            ...
            "restructuring (splitting, joining, reordering clauses, recasting a statement as a "
            "question or vice versa, a blunter pivot, a deliberate fragment) toward the writer's real "
            "calibrated construction is expected wherever SENTENCE STRUCTURE/VOICE OBSERVATIONS/ANCHOR "
            "SENTENCES above show that's genuinely closer to their voice, even where the input's "
            "existing sentence boundaries were already grammatically fine. Lexical fidelity means: "
            "don't swap their actual words for synonyms, and don't add polish or hedging that isn't "
